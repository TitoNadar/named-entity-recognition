...
def run_model(text, model, tokenizer, predict=True):
	if predict:
		output = ACE.predict(text, model, tokenizer)
...
def transform_result(result):
	...
	for token, label in result:
		output.append({"entity":label, "text":str(token)})
	return output